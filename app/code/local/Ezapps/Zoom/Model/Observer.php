...
class Ezapps_Zoom_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function clearProduct($product_event) {
        ...
        foreach ($stores as $store_id) {
			$uris = Mage::getModel('core/url_rewrite')
				->getCollection()
			...
			foreach ($uris as $uri) {
				Mage::getModel('ezzoom/page')->getCollection()
					->addFieldToFilter('store_id', array('eq' => $store_id))
					->addFieldToFilter('uri', array('like' => "%/" . $uri->getRequestPath().'%'))->delete();
			}
			...
			$uris = Mage::getModel('core/url_rewrite')->getCollection()
				->addFieldToFilter('store_id', array('eq' => $store_id))
				->addFieldToFilter('target_path', array('like' => "review/product/list/id/" . $product['entity_id']));
    ...
    public function clearCMS($cms_page) {
		...
		foreach ($stores as $store_id) {
			Mage::getModel('ezzoom/page')->getCollection()
				->addFieldToFilter('store_id', array('eq' => $store_id))
				->addFieldToFilter('uri', array('like' => "%/{$orig_page['identifier']}%"))->delete();
            ...
            Mage::getModel('ezzoom/page')->getCollection()
				->addFieldToFilter('store_id', array('eq' => $store_id))
				->addFieldToFilter('uri', array('like' => '%cms/page/view/page_id/' . $orig_page['page_id'].'%'))->delete();
		}
    }
}
